          ...
          renderMobileCard={(f) => (
            <div className="space-y-2 mt-1">
              <div className="flex items-start justify-between">
                <div>
                  <div className="font-medium text-body-md text-foreground">{f.guestName}</div>
                  <div className="font-mono text-[10px] text-muted-foreground mt-0.5">{f.id}</div>
                </div>
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium uppercase border shrink-0 ${STATUS_COLORS[f.status]}`}>
                  {f.status}
                </span>
              </div>
              <div className="flex items-center justify-between text-body-sm bg-surface-2 p-2 rounded-sm border border-border/50">
                <div className="flex flex-col">
                  ...
                  <span className="font-mono font-medium text-foreground">{f.roomNumber}</span>
                </div>
                <div className="flex flex-col text-center">
                  <span className="text-[10px] text-muted-foreground uppercase tracking-wide">Total</span>
                  <span className="font-mono text-foreground">{formatINR(folioTotal(f))}</span>
                </div>
                <div className="flex flex-col text-right">
                  <span className="text-[10px] text-muted-foreground uppercase tracking-wide">Balance</span>
                  <span className="font-mono font-semibold text-foreground">{formatINR(folioBalance(f))}</span>
                </div>
              </div>
            </div>
          )}
